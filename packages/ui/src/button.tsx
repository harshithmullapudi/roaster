...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded transition-colors focus-visible:outline-none focus-visible:shadow-none disabled:pointer-events-none disabled:opacity-50 dark:focus-visible:ring-slate-300",
  {
    variants: {
      ...
      size: {
        default: "h-(--btn-h-default) rounded px-2 py-1",
        sm: "h-(--btn-h-sm) rounded px-2 py-2",
        xs: "h-(--btn-h-xs) rounded px-1 py-1",
        lg: "h-(--btn-h-lg) px-4 py-2",
        xl: "h-(--btn-h-xl) rounded px-8 text-base",
        "2xl": "h-(--btn-h-2xl) rounded px-8",
        icon: "h-(--btn-h-icon) w-(--btn-w-icon)",
      },
      full: {
        false: "w-auto",
        true: "w-full",
