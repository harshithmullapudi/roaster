...
/*
 * Two counts, never summed. A session waiting on a person is the one you can
 * actually act on, so it keeps its own amber dot rather than being folded into
 * "running" — and it is still, because the pulse means work is happening.
 */
export function SessionCounts({
  ...
  if (running <= 0 && needsInput <= 0) return null;
